...
> ## 2026-09-03 — **EVERY ENTRY BELOW DATED TODAY SAYS "BUILT". TWENTY OF TWENTY-ONE SHOULD SAY "UNSEEN".**
>
> **The USER reported the day's work incomplete, and the record is part of why.** *"Built" has been
> used to mean two different things — **the code exists** and **the feature works** — and only the
> first is established for twenty of the twenty-one units recorded today.*
>
> **A join census over all 21 found NO orphans left:** every unit passes criterion **(b)** —
> something reads what it produces — after #21 wired #18's (`api.restoreEntityOutcome`, read at
> `gui.nim:3995`). **A-48 is the exception and is not "unseen" but structurally blocked**: owlkettle's
> `Label` declares no events and `activate-link` exists nowhere in the toolkit, so the click has no
> owner.
>
> **Only the composer has been seen by a human** (entries 10:21–11:24). **It passes only because the
> USER used it — and it took FOUR fixes across four rounds, none of which any check in this project
> could detect.** `bin/jenova --check` was green on all three unusable builds.
>
> **The other twenty are now in exactly the state that one was in before the USER looked at it:**
> 13b's three parts, 12c (A-3/A-4/A-5), A-69, A-26, A-17, A-24, A-20, A-16, A-19, A-18-1 and its
> wiring, A-18-2, A-18-3, A-70, and 12d. **Read every one of them as IMPLEMENTED, UNSEEN.**
>
> > ### And as of 13:19 the situation is worse than unseen: **the product SEGFAULTS.**
> >
> > `bin/jenova --check` exits **139**, and there are **four cores in `/var/coredumps` between 13:15
> > and 13:19**. **Frame 0 is `gui.mdBlock`** — the markdown block renderer — reached
> > `run` → `build` → `view` → `mainArea` → `messageBody` → `mdBlock`, **on the first view build.**
> > `nimble core` is also red on `pdf.nim:196`, a half-applied A-61.
> >
> > **So the window does not reach its first frame.** The join census above was run against source
> > shape while the binary was crashing. **Rule 17 says a green `--check` proves the window reaches
> > its first frame and nothing more — twenty entries rested on that, and it is now not even true.**
>
...
> ### CORRECTION 2026-09-03 — **"links are rendered" overstates it, and the missing half is exactly the USER's report.**
>
> **What is true:** `markdown.nim:149` emits `<a href="…">`, the scheme allowlist is real, enforced
> at markup-generation time, and asserted from both sides. **What is not established: that clicking
> one does anything.**
>
> **There is no link-activation handler anywhere in `src/` — zero hits for `activate-link`,
> `activateLink`, `gtk_show_uri`, `show_uri`, `UriLauncher` and `openUri`, verified across the whole
> tree.** The unit was recorded with "no `gui.nim` change was needed" on the reasoning at
> `markdown.nim:47` that GTK "hands an activated `<a href>` to the desktop URI handler". **That is a
> claim about a GtkLabel default that nobody in this project has established, asserted or seen.**
>
> **So this is not "unobserved" — it is UNKNOWN MECHANISM**, and the distinction matters:
> *unobserved* means we know what it does and have not watched it; *unknown mechanism* means we
> assumed a toolkit behaviour and built on it. **A user seeing a blue underlined link that does
> nothing is precisely the report that came back.**
>
> **SETTLED, and it is a NEGATIVE RESULT: owlkettle offers no route to a link click at all.**
> `Label` declares **zero events** — `text`, `xAlign`, `yAlign`, `ellipsize`, `wrap`, `useMarkup`
> and their hooks, and not one `proc` — and **`activate-link` appears nowhere in the whole of
> owlkettle.** So there is nothing in this program that can handle, enforce or even *observe* a
> click; as shipped it reaches `GtkLabel`'s built-in default and **nothing here participates.**
> **Reclassified: the rendering half is done and asserted; the activation half is BLOCKED ON A
> RENDERABLE and was never established.**
>
> **The allowlist is untouched by that and must not be doubted on account of it** — it is enforced
> at markup-generation time, with 24 assertions, four hostile schemes refused with their link text
> surviving, and a link inside a code span not linkified. **The security property and the click are
> independent, and only the click is unestablished.**
>
> **A renderable owning the `GtkLabel` is not scope creep — it is D-BR, the sanctioned pattern**,
> and the reason `NeuralCanvas`, `NvimTerminal`, `SourceCode`, `DropZone`, `AutoScroll`,
> `ContentScroll` and `DraftView` all exist. **The failure this project paid three broken builds for
> was *reaching around* owlkettle; owning the widget is the opposite move.**

### 2026-09-03 12:02 — **Step 12e-2: markdown links and images are MARKED UP behind a scheme allowlist; activation unverified (A-48).** `markdown.inlineMarkup` ran a code-span lift and four emphasis passes and **never examined a bracket**, so `[RFC 7231](https://…)` reached the Pango Label as literal characters and `![alt](url)` did too — a search for `href` across `src/` returned zero. Links are now lifted **between the code-span pass and the emphasis passes, and that position is the whole design:** *after* the code lift, because `` `[a](b)` `` must stay literal and the code span has already left the string; *before* emphasis, because a URL is full of characters those passes eat — `https://host/a__b__c` is one pair away from turning half a sentence bold. The href leaves the string entirely into a `\x01`-delimited placeholder and only the link **text** stays inline, which is why `[**bold**](url)` still emphasises; `\x01`/`\x02` rather than the code pass's `\x00` so the two schemes cannot collide. **The allowlist is the security decision and it is the right way round:** only `http://` and `https://` become `<a href>`, because **GTK hands an activated href to the desktop URI handler**, so `[click](file:///etc/passwd)` written by a model would otherwise be an instruction to the desktop — and a denylist cannot be completed against an open-ended set of schemes. Everything else renders as its own text with no destination: a visible loss rather than a link that quietly does something else. **Stated limit, not a gap: an image is its alt text, linked, never fetched** — displaying a remote image means a network request from inside a render path, which is B-01's leak and Step 7c's per-frame rule at once, and that is a USER decision rather than a side effect of this fix. **24 assertions added to `markdown-selftest` (49 now, from 25), both sides of the allowlist varying only the DATA (D-BX):** an https link becomes an anchor with the exact href and text; **four hostile schemes — `file://`, `javascript:`, `data:` and a bare relative path — each produce no anchor while the link text survives**; a link inside a code span is not linkified, **which is the assertion that proves the pass ordering**; `&` stays escaped in the attribute and a `"` cannot end it early; no placeholder control character leaks into the markup handed to Pango; and brackets that are not a link are byte-for-byte untouched. **A pass that linkified everything fails the four hostile cases, one that linkified nothing fails the first, and one that ran before the code lift fails the code-span case.** Green, verified independently: `nimble core` 0, `nimble gui` 0, `--check` 0, `markdown-selftest` PASS at 49 assertions. **No `gui.nim` change was needed** — the allowlist is enforced at markup time, so GTK's handler can only ever receive an http/https URI. **Unobserved: that a rendered link is clickable and opens is a USER screen run.** Files: `markdown.nim`, `jenova_core.nim`.
