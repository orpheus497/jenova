...
| File | Lines | Role |
|---|---|---|
| `proxy.lua` | 1748 | The Intelligence Proxy; the client-facing HTTP surface and event loop. **Superseded for serving as of 2026-08-31 (N-S5a)** — every route it answers is now served by the Nim core, except `/api/workspaces`, which is dead and deliberately dropped. **It is not yet removable:** `lib/proxy.lua:1225-1400` still holds the completion pipeline — intents, RAG, web search, personas, tool stripping and the cache intercept — which is unported (**N-30**, stage N-S5c) |
