...
| File | Lines | Role |
|---|---|---|
| `proxy.lua` | 1748 | The Intelligence Proxy; the client-facing HTTP surface and event loop. **Still load-bearing as of 2026-08-31** — it is the only server for `/api/storage/*` (4 routes), `/api/workspaces` and `/infill` (N-29). `/api/db/*` and `/api/fs/*` are now reproduced in the Nim core |
