...
| **D-N** | **Single binary — the GUI links the Nim core in-process.** Implies direct `llama.cpp` linkage |
...
| ~~N-28~~ | **Closed 2026-08-31 by D-AD + Q-27.** The runtime home moved to `$HOME/Jenova` at all eleven code sites, and `paths.resolve` now refuses `$HOME/JCA` outright unless `JENOVA_ALLOW_DEPLOYED=1`. See `PROGRESS.md` |
...
| **N-29** | **PARTIAL — `/infill` and `/v1/health` fixed at N-S4c 2026-08-31; `/api/storage/*` still outstanding.** `/api/storage/*`, `/api/workspaces` and `/infill` are unported — `lib/proxy.lua` is NOT retired.** Verified by probing a running core, not by reading: `GET /api/storage` → 404, `GET /api/storage/<path>` → 404, `GET /api/workspaces` → 404, `POST /infill` → 405 (`routes.nim` never classifies it, so it lands in the static class), `GET /v1/health` → 400 (classified to completion, which fails to parse a body). `POST` and `DELETE /api/storage/<path>` are likewise absent; the DELETE needs **`fs_sync.trash_path`, the one function of 13 not ported**. **Why this was missed: the audit recorded route *families* it noticed and never diffed them against the implementation.** A route inventory is one loop against a running binary and had never been run | `lib/proxy.lua:1009,1041,1068,1096,1126`; `lib/fs_sync.lua:281`; `src/jenova/routes.nim:66-79` |
| **N-S5a** | **PARTIAL, not complete — corrected 2026-08-31.** Done and tested: the `fs_sync` mirroring (N-27) and the four `/api/fs/*` routes (N-20), 31 assertions passing. **Outstanding: the five routes in N-29 and `fs_sync.trash_path`.** `lib/proxy.lua` cannot be retired until they land |
| ~~N-S5a-partial~~ | **Landed 2026-08-31** — `fssync.nim`, the ten mirroring call sites, the four `/api/fs/*` routes, and `tests/test_api_fs.sh` (31 assertions, PASS). N-27 and N-20 closed. See `PROGRESS.md` |
