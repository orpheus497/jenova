...
| **A-76** | `[V]` **A-52 WAS RIGHT. A census said it under-reported six-fold, that census was published to three sessions as verified fact, and it was wrong. All three states are recorded because the middle one is the finding.** **State 1 — A-52 says seven dead exported procs. State 2 — a census said 42 of 273** and was reproduced independently by a second session and then by this one, reaching 41, 42 and 42; **agreement between three sessions using the same flawed method felt exactly like corroboration.** **State 3 — the defensible figure is ELEVEN, and A-52's seven are all correct**, so it under-reports by four and its only error is calling the libdbus set "three" when there are four. **What the flawed method could not see: an export reached through a SAME-FILE WRAPPER.** The decisive counter-example is `pipeline.entryFor` — it appears in no file but its own, so the census called it dead, **and it runs on every frame**, reached from `view` via `attachmentsFor` (`gui.nim:1919`) and `extraNodeFor` (`:1799`). **The corrected method — declaration is the only occurrence anywhere in `src/`, own file included — has its own flaw, found here:** it counts comment and substring matches, so `nvimctl.alive` was excluded by the words *"keep-alive"* and *"is this pid alive?"*. **Hence eleven and not ten, and hence a word rather than a number** (rule 22): *roughly a dozen, and A-52 names most of them.* **The genuinely dead set:** `db.columnNames`, `fssync.scopeDir`, `models.countDevices`, `pipeline.parseAttachments`, `rag.indexFile`, `tray.isRegistered`, `nvimctl.alive`, and four libdbus bindings. **~32 further procs are exported without needing to be — tidiness, not a feature gap.** **The category that actually matters is still uncounted:** *(c)* **a missing consumer — a half-built feature wearing a finished label.** `api.restoreEntityOutcome` was one, found by direct check rather than by census, **and it has since been wired.** *Recorded at length because "the trackers under-report by six-fold" would have been A-52's own recorded failure — the "seven `make` references" that were two — committed inside the audit built to catch it.* Also measured, and unaffected by any of the above: **The three categories are what matter, and only the third is the USER's complaint:** *(a)* **internal-only** — wants the `*` dropped, harmless: `fssync.isValidUuid`, `fssync.sanitize`, `pdf.readHex`, `pdf.readLiteral`, `rag.chunkText` and the seven `dbus_*` C bindings. *(b)* **genuinely dead** — delete, git is the archive (**D-AM**). *(c)* **a missing consumer — a half-built feature wearing a finished label**, which is `api.restoreEntityOutcome`, the `ParseMemo` read chain under A-6, and however many of the remaining 42 turn out to be the same shape. **Nobody has ever counted (c) separately, and both instances we have were found by accident rather than by search.** Also measured: **41 exported procs with no caller anywhere outside their own file** — A-52 names seven of them (`pipeline.parseAttachments`, `rag.indexFile`, `db.columnNames`, `fssync.scopeDir`, `tray.isRegistered`, `models.countDevices`, `pipeline.forget`) and **thirty-four are unnamed by any tracker**; **53 stub / placeholder / no-op / "for now" markers**, 23 of them in `gui.nim`; **7 conf keys with no consumer** (A-62/A-63, now including `JENOVA_HEALTH_TIMEOUT`); **4 settings fields drawn, stored and wired to nothing** (A-29); plus `Prepared.trimmed` written and never read (A-8) and `handedOff` dead (A-38). **Two were manufactured today:** `api.restoreEntityOutcome`, orphaned at birth, and **`fssync.underRoot`, left dead by A-19** — every call site replaced by `resolvedUnderRoot`, and the only surviving reference is the comment naming it. **This row is the measured shape of "incomplete" and should be the thing a completeness pass is checked against**, rather than any session's sense of how much is done. |
...
| ID | V | What it is |
|---|---|---|
| **A-6** | `[V]` | **The G-40 memos removed the per-frame parse but not the per-frame copy.** `markdown.blocksFor`'s hit path is `return memo.blocks[id]` (`src/jenova/markdown.nim:232`) and `pipeline.entryFor`'s builds a tuple from `memo.atts[id]` (`src/jenova/pipeline.nim:859`); both read out of a table that retains ownership, so under ARC the value is `=copy`'d — deep-copying every string, and for an image attachment the whole base64 payload. Both are called from `view` (`gui.messageBody`, `gui.mainArea`). **If this holds, `view` still does work proportional to the transcript on every frame — the complexity L7 forbids — and only the constant fell.** The instrument aimed at it is blind: `parses` counts parses, not copies, so `attach-selftest`'s `mm.parses == 1` stays green regardless. **A-6 STANDS ON BOTH HALVES. Re-verified 2026-09-03 after two failed attempts to refute it, both mine.** `markdown.blocksFor` is called per frame from `view` at `gui.nim:2935` (every message) and `:3410` (the open note). `pipeline.entryFor` is reached from `view` through its same-file wrappers — `attachmentsFor` at `gui.nim:1919` and `extraNodeFor` at `:1799`. **Both call sites are real and the finding is unchanged.** *Recorded because the two failures are more instructive than the result:* **(1)** the first check of `blocksFor` was piped through `head`, which truncated the `gui.nim` hits away and read as "no caller in the GUI"; **(2)** the second searched for wrappers named `attsFor`/`nodeFor`, **which do not exist** — they are `attachmentsFor`/`extraNodeFor` — and a search for a name that was never in the tree returns zero and looks like proof. **Both are rule 20, committed twice in ten minutes by the session that recorded rule 20 four hours earlier: a search returning nothing is not evidence until you have confirmed the search could have found something.** |
...
| **A-33** | `[V]` | **Sidebar filters allocate a lowercased copy of every name on every frame, and `convsIn` re-runs the whole conversation filter once per tree node.** `gui.visibleConvs` (`gui.visibleConvs`) either copies `app.convs` wholesale or allocates `c.name.toLowerAscii` per conversation; `convsIn` calls it once per invocation and `view` invokes it at four nesting levels. Not large today, and recorded only because the module is otherwise scrupulous about this exact rule. |
...
| **A-42** | `[V]` | **Restoring from the Trash panel performs embedding HTTP round trips on the GTK thread.** `gui.restoreFromTrash` runs on the GTK thread (`gui.restoreFromTrash`) and `api.restoreItem` re-indexes; for a restored *conversation* it loops every assistant turn calling `rag.indexExchange`, each an HTTP POST with a 30-second client timeout (`src/jenova/rag.nim:151`). **Restoring a long conversation with the embedder slow or down freezes the window for turns × up to 30 s.** |
| **A-43** | `[V]` | **`workspace.contextFor` reads every note's and every file asset's full content on the GTK thread, on every send.** Called from `gui.postConversation` (`gui.postConversation`), with `allNotes` and `allFiles` both selecting `content` (`src/jenova/workspace.nim:79-91`). Because `fileAttachmentsAsArtefacts` files each chat attachment as a `fileAssets` row, **this set grows with use and nothing bounds it**, so Send gets progressively more expensive. |
...
- **Stale comments describing deleted code** — `server.nim:83-85` describes an in-process
  ...
  terminal; **four** `libdbus` functions are bound against the module's explicit no-dead-surface
