...
This is treated as a sub-repository with its own lifecycle, while everything else—including the **jenova-ui** desktop manager, and **jca_web**—is built-in and directly wired into the Jenova core.
...
## 2. Shared Runtime State (`~/JCA/.system`)
Transient state, such as process PIDs, active model metadata, and session tokens, is stored in the hidden `.system` directory within your `JCA_HOME` (typically `~/JCA`).
- `jenova-ca` (the daemon) writes PIDs here.
- `jenova-ui` (the TUI/Tray) reads these PIDs to monitor health.
- Launchers check these files to auto-start the backend if it's not already running.

## 3. Communication Loop
Components interact via a multi-port bridge:
- **Port 8080 (Proxy)**: The "brain" of the system. It handles RAG, tool execution, and workspace management. It serves as the gateway for the WebUI and API clients.
