...
2. **FOSS Compliance (Permissive Primary):** Rely on Free and Open-Source Software under permissive, non-copyleft licenses (MIT, BSD, zlib, Public Domain), with MIT and BSD preferred. GPL, LGPL, or any other copyleft license is strictly prohibited. Zero proprietary dependencies.
...
5. **Architectural Pipeline Adherence:** Before making code changes, consult the data-flow interactions in `BLUEPRINT.md`. Do not bypass the C11/Nim contract or introduce circular module dependencies that violate the engine architecture. `BLUEPRINT.md` is the single authoritative architecture reference — `ARCHITECTURE.md` (root) and `docs/ARCHITECTURE.md` are human-facing summaries derived from it; if they ever disagree, `BLUEPRINT.md` wins and the others must be updated to match.
6. **Zero Circumvention / No Fabricated Completion:** It is strictly forbidden to report, imply, or document a feature, fix, or test as complete/working unless it has actually been verified (build passes, and — where a windowed session is unavailable — the specific manual verification step is recorded as outstanding, never silently assumed). It is strictly forbidden to introduce placeholders, stubs, simulated/hardcoded stand-ins for real logic, or any code whose purpose is to make something *appear* to work by bypassing the actual system it should run through (e.g., hardcoding content that should be produced by using the editor; faking a return value instead of implementing the call; duplicating C-owned state in Nim instead of reading it live). This applies to code and to `.devdocs/` alike — a tracker entry claiming "Completed" for something not actually done is itself a violation. Every session must run a `.devdocs/PLANS.md` "Codebase Integrity Standard" pass (see that file) proportional to the area touched, and record any newly found instance of this class — placeholder, stub, false implementation, dead/orphaned code, tech debt, hallucinated or unverified logic, or any other circumvention — in `TODOS.md`/`DECISIONS_LOG.md` rather than leaving it undocumented, even if fixing it is out of scope for the current step.

7. **No Backward Compatibility:** The engine is pre-release — there are no users, no shipped `.dbc` cartridges, and no legacy files. When a binary format, struct layout or API changes, it changes **outright**, and the affected `test_roms/` projects are converted **by hand**. It is forbidden to write migrators, `_vN` translation functions, pinned copies of superseded structs, version-fallback branches, or error messages instructing the user to run a converter. A version mismatch in a loader must fail loudly and do nothing else. Compatibility scaffolding costs real code and real maintenance to protect files that do not exist, and it accretes into the engine permanently.
