#!/bin/sh
# install.sh: Jenova Cognitive Architecture — dGPU-Only 3B Profile Installation (Optane)
...
echo ""
printf "${_B}╔══════════════════════════════════════════════════════╗${_N}\n"
printf "${_B}║  Jenova CA — 3B dGPU-Only Optane Profile Install     ║${_N}\n"
printf "${_B}╚══════════════════════════════════════════════════════╝${_N}\n"
echo ""
...
if mount | grep -q "on $MOUNT_DIR "; then
    ...
    warn "  Run: sudo ./hardware-profiles/Optane/dgpu/i5-1135g7-3b/jenova-setup"
...
_agent_model="${MODEL_PATH:-$JENOVA_ROOT/models/agent/Qwen2.5-Coder-3B-Instruct-Q8_0.gguf}"
if [ -f "$_agent_model" ] || [ -L "$_agent_model" ]; then
    ok "Agent model (3B): $(basename "$_agent_model")"
    ...
    echo "     Place a 3B GGUF model in models/agent/ or symlink from $MOUNT_DIR"
    echo "     e.g.: ln -s $MOUNT_DIR/Qwen2.5-Coder-3B-Instruct-Q8_0.gguf models/agent/"
    ERRORS=$((ERRORS + 1))
fi
...
# ---------------------------------------------------------------------------
info "Deploying 3B dGPU-only profile configuration..."
...
if [ -f "$_PROFILE_CONF" ]; then
    ...
    ok "Deployed 3B dGPU-only profile to etc/jenova.conf"
else
...
echo ""
printf "${_B}══════════════════════════════════════════════════════${_N}\n"
printf "${_B}  Installation Summary (3B dGPU-Only Optane Profile)${_N}\n"
printf "${_B}══════════════════════════════════════════════════════${_N}\n"
echo "  Errors:   $ERRORS"
...
echo ""
info "Next steps:"
echo "  1. Run system tuning (once):  sudo ./hardware-profiles/Optane/dgpu/i5-1135g7-3b/jenova-setup"
echo "  2. Ensure swap mount is up:   df -h /mnt/jenova-models"
echo "  3. Copy 3B model to mount:    cp <model>.gguf /mnt/jenova-models/"
