...
## Function purpose: write the window's one-line message, from anywhere.
##
...
viewable App:
  ...
  ## not time out — and the Retry button below has to have the same lifetime as
  ## its handler, which a toast does not.
  noticeIsError: bool
  ...
  result = gui:
    ...
    AdwWindow:
      ...
      ShortcutHost:
        ...
        # Phase 3, report 05. `AdwToastOverlay` is not in owlkettle at all — see
        # `toast.nim` — so this is the escape hatch, and it wraps the whole
        # window because that is where libadwaita floats a toast: bottom centre,
        # over everything, out of the layout.
        #
          ...
          Overlay:
            ...
            Flap {.addOverlay.}:
              ...
              Box(orient = OrientY):
                ...
                Box(orient = OrientX, spacing = 8) {.expand: false.}:
                  # Phase 3. **This row is now the error surface and nothing
                  # else.** Every confirmation — "note saved", "attached X",
                  # "settings saved" — goes to `ToastOverlay` above, appears
                  # over the transcript and times out, which is what a
                  # confirmation is for. What was wrong with putting them here
                  # is that they never left: "note saved" sat under the
                  # composer until the next unrelated action replaced it, and
                  # by then it was describing something the user did minutes
                  # ago.
                  #
                  # An error stays, and stays *here*, for two reasons. It is
                  # the thing you may have to act on, and a message that times
                  # out is a message you can miss; and the Retry button below
                  # has to have the same lifetime as its handler, which a
                  # toast — outliving several redraws while owlkettle replaces
                  # the `EventObj` under it — does not. See `toast.nim`.
                  margin = (if app.noticeIsError and app.notice.len > 0 and
                               not app.editorOpen: 8 else: 0)
                  Label {.expand: true.}:
                    # It has nothing to say about the editor page either, and a
                    # stale line under a full-screen editor reads as an error in
                    # it.
