...
export function parseMcpServerSettings(
  ...
  if (typeof rawServers === "string") {
    ...
    try {
      ...
      console.warn(
        "[MCP] Failed to parse mcpServers JSON, ignoring value:",
        error,
      );
 ...
 */
export function isCodeResource(
...
export function getResourceIcon(
  ...
  if (
    mime.startsWith(MimeTypePrefix.IMAGE) ||
 ...
 */
export function getResourceTextContent(
  content: MCPResourceContent[] | null | undefined,
): string {
  if (!content) return "";
  return content
    .filter(
      (c): c is { uri: string; mimeType?: MimeTypeUnion; text: string } =>
        "text" in c,
    )
    .map((c) => c.text)
...
export function getResourceBlobContent(
  ...
  return content.filter(
    (c): c is { uri: string; mimeType?: MimeTypeUnion; blob: string } =>
      "blob" in c,
  );
}
