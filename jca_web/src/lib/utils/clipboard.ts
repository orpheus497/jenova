...
export function formatMessageForClipboard(
  ...
  const clipboardAttachments: ClipboardAttachment[] = textAttachments.map(
    (att) => {
...
function isValidMcpPromptAttachment(obj: unknown): obj is {
  ...
  return (
    (record.type === AttachmentType.MCP_PROMPT ||
      record.type === "MCP_PROMPT") &&
...
export function hasClipboardAttachments(clipboardText: string): boolean {
  ...
  return (
    parsed.textAttachments.length > 0 || parsed.mcpPromptAttachments.length > 0
  );
}
