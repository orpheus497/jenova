...
const MIGRATION_DONE_KEY = "jenova-webui-migration-v2-done";
...
function parseLegacyToolCalls(content: string): ParsedTurn[] {
  ...
  const regex = new RegExp(
    LEGACY_AGENTIC_REGEX.COMPLETED_TOOL_CALL.source,
    "g",
  );
  ...
  for (const message of allMessages) {
    ...
    if (!hasLegacyMarkers(message)) {
      ...
      if (message.content?.includes(LEGACY_REASONING_TAGS.START)) {
        const { reasoning, cleanContent } = extractLegacyReasoning(
          message.content,
        );
    ...
    const firstTurnToolCalls = firstTurn.toolCalls.map((tc, i) => {
      const existing =
        existingToolCalls.find((e) => e.function?.name === tc.name) ||
        existingToolCalls[i];
      return {
    ...
    await DatabaseService.updateMessage(message.id, {
      ...
      reasoningContent: reasoning || undefined,
      toolCalls:
        firstTurnToolCalls.length > 0 ? JSON.stringify(firstTurnToolCalls) : "",
    });
    ...
    for (let turnIdx = 1; turnIdx < turns.length; turnIdx++) {
      ...
      const assistantMsg = await DatabaseService.createMessageBranch(
        {
          ...
          toolCalls:
            turnToolCalls.length > 0 ? JSON.stringify(turnToolCalls) : "",
          model: message.model,
        },
        currentParentId,
      );
      ...
      for (let i = 0; i < turn.toolCalls.length; i++) {
        ...
        const toolCallId =
          turnToolCalls[i]?.id || `legacy_tool_${toolCallIdCounter + i}`;
    ...
    if (message.children.length > 0 && currentParentId !== message.id) {
      for (const childId of message.children) {
        ...
        if (child.role !== MessageRole.TOOL) {
          ...
          const newParent = await DatabaseService.getConversationMessages(
            convId,
          ).then((msgs) => msgs.find((m) => m.id === currentParentId));
