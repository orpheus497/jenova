...
class MCPResourceStore {
  private _serverResources = $state<SvelteMap<string, MCPServerResources>>(
    new SvelteMap(),
  );
  private _cachedResources = $state<SvelteMap<string, MCPCachedResource>>(
    new SvelteMap(),
  );
  private _subscriptions = $state<SvelteMap<string, MCPResourceSubscription>>(
    new SvelteMap(),
  );
  ...
  updateAttachmentContent(
    ...
    this._attachments = this._attachments.map((att) =>
      att.id === attachmentId
        ? { ...att, content, loading: false, error: undefined }
        : att,
    );
  }
    ...
    for (const [serverName, serverRes] of this._serverResources) {
      const resource =
        ...
        serverRes.resources.find(
          (r) => normalizeResourceUri(r.uri) === normalizedUri,
        );
...
export const mcpResourceAttachmentCount = () =>
  mcpResourceStore.attachmentCount;
