...
class ModelsStore {
   ...
   */
  private modelPropsCache = new TTLCache<string, ApiJenovaCppServerProps>({
   ...
   */
  getModelProps(modelId: string): ApiJenovaCppServerProps | null {
   ...
   */
  async fetchModelProps(
    modelId: string,
  ): Promise<ApiJenovaCppServerProps | null> {
  ...
  async fetchModalitiesForLoadedModels(): Promise<void> {
    ...
    const propsPromises = loadedModelIds.map((modelId) =>
...
export const selectedModelContextSize = () =>
  modelsStore.selectedModelContextSize;
