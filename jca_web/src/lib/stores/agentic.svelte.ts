...
class AgenticStore {
  ...
  getConfig(
    ...
    const maxTurns =
      Number(settings.agenticMaxTurns) || DEFAULT_AGENTIC_CONFIG.maxTurns;
    ...
    return {
      enabled:
        ...
        DEFAULT_AGENTIC_CONFIG.enabled,
      maxTurns,
      maxToolPreviewLines,
    };
  }
  ...
  async runAgenticFlow(params: AgenticFlowParams): Promise<AgenticFlowResult> {
    ...
    if (!initialized) {
      console.log(
        "[AgenticStore] MCP not initialized, falling back to standard chat",
      );
    ...
    if (tools.length === 0) {
      console.log(
        "[AgenticStore] No tools available, falling back to standard chat",
      );
      return { handled: false };
    }

    console.log(
      `[AgenticStore] Starting agentic flow with ${tools.length} tools`,
    );
  ...
  private async executeAgenticLoop(params: {
    ...
    for (let turn = 0; turn < maxTurns; turn++) {
      ...
      if (turnToolCalls.length === 0) {
        ...
        const finalTimings = this.buildFinalTimings(
          capturedTimings,
          agenticTimings,
        );
      ...
      for (const toolCall of normalizedCalls) {
        ...
        const { cleanedResult, attachments } =
      ...
      if (turnStats.toolCalls.length > 0) {
        ...
        const intermediateTimings = this.buildFinalTimings(
          capturedTimings,
          agenticTimings,
        );
