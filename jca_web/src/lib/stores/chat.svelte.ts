...
class ChatStore {
  ...
  private conversationStateTimestamps = new SvelteMap<
    string,
  ...
  hasPendingDraft(): boolean {
    return (
      Boolean(this._pendingDraftMessage) || this._pendingDraftFiles.length > 0
    );
  }
  ...
  async addMessage(
    ...
    if (parent === "-1") {
      ...
      if (am.length > 0) parentId = am[am.length - 1].id;
        ...
        const all = await conversationsStore.getConversationMessages(
          activeConv.id,
        );
    ...
    try {
      ...
      if (firstActiveMessage) {
        ...
        await DatabaseService.updateMessage(rootId, {
          children: [
            ...updatedRootChildren.filter(
              (id: string) => id !== systemMessage.id,
            ),
            systemMessage.id,
          ],
        });
  ...
  async regenerateMessage(messageId: string): Promise<void> {
    ...
    try {
      ...
      const assistantMessage =
        await this.createAssistantMessage(parentMessageId);
      conversationsStore.addMessageToActive(assistantMessage);
      await this.streamChatCompletion(
        conversationsStore.activeMessages.slice(0, -1),
        assistantMessage,
  ...
  async continueAssistantMessage(messageId: string): Promise<void> {
    ...
    try {
      ...
      const conversationContext = conversationsStore.activeMessages.slice(
        0,
        idx,
      );
      ...
      await ChatService.sendMessage(
        contextWithContinue,
        {
          ...
          onComplete: async (
            ...
            AudioService.speak(fullContent);
            AudioService.notify(fullContent);
  ...
  async editMessageWithBranching(
    ...
    try {
      ...
      const hasChildren = dbMsg
        ? dbMsg.children.length > 0
  ...
  private getContextTotal(): number | null {
    ...
    const activeState = activeConvId
      ? this.getProcessingState(activeConvId)
      : null;
  ...
  private getApiOptions(): Record<string, unknown> {
    ...
    if (currentConfig.useThinking) apiOptions.useThinking = true;

    if (currentConfig.useAudioVoice) apiOptions.useAudioVoice = true;
...
export const getChatStreaming = (convId: string) =>
  chatStore.getChatStreamingPublic(convId);
export const isChatLoading = (convId: string) =>
  chatStore.isChatLoadingPublic(convId);
