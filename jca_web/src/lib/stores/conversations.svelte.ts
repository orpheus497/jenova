...
class ConversationsStore {
  ...
  pendingMcpServerOverrides = $state<McpServerOverride[]>(
    ConversationsStore.loadMcpDefaults(),
  );
  ...
  async createConversation(name?: string): Promise<string> {
    ...
    const conversation =
      await DatabaseService.createConversation(conversationName);
  ...
  async loadConversation(convId: string): Promise<boolean> {
    try {
      ...
      if (conversation.currNode) {
        const allMessages =
          await DatabaseService.getConversationMessages(convId);
      ...
      if (options?.deleteWithForks) {
        ...
        while (queue.length > 0) {
          ...
          for (const c of this.conversations) {
            if (
              c.forkedFromConversationId === parentId &&
        ...
        if (
          ...
          idsToRemove.has(this.activeConversation.id)
        ) {
  ...
  async updateConversationTitleWithConfirmation(
    ...
    try {
      ...
      if (
        currentConfig.askForTitleConfirmation &&
      ...
      console.error(
        "Failed to update conversation title with confirmation:",
        error,
      );
      return false;
    }
  ...
  async navigateToSibling(siblingId: string): Promise<void> {
    ...
    await DatabaseService.updateCurrentNode(
      this.activeConversation.id,
      currentLeafNodeId,
    );
    this.activeConversation = {
    ...
    if (rootMessage && this.activeMessages.length > 0) {
      ...
      if (
        ...
        (!currentFirstUserMessage ||
          ...
          newFirstUserMessage.content.trim() !==
            currentFirstUserMessage.content.trim())
      ) {
  ...
  async setMcpServerOverride(
    ...
    const currentOverrides = (
      this.activeConversation.mcpServerOverrides || []
    ).map((o: McpServerOverride) => ({
      serverId: o.serverId,
      enabled: o.enabled,
    }));
  ...
  generateConversationFilename(
    ...
    const trimmedConvId =
      conversation.id?.slice(0, EXPORT_CONV_ID_TRIM_LENGTH) ?? "";
  ...
  downloadConversationFile(
    ...
    const downloadFilename =
      filename ?? this.generateConversationFilename(conversation, msgs);
    ...
    return new Promise((resolve, reject) => {
      ...
      input.onchange = async (e) => {
        ...
        try {
          ...
          const result =
            await DatabaseService.importConversations(importedData);
          toast.success(
...
export const isConversationsInitialized = () =>
...
export function buildConversationTree(
  ...
  for (const conv of convs) {
    if (conv.forkedFromConversationId) {
      ...
      const siblings =
        childrenByParent.get(conv.forkedFromConversationId) || [];
