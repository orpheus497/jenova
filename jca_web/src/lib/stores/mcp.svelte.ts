...
class MCPStore {
   ...
   * The endpoint is only registered when jenova-server runs with --webui-mcp-proxy.
   */
  ...
  #parseServerSettings(rawServers: unknown): MCPServerSettingsEntry[] {
    ...
    return parsed.map((entry, index) => {
      ...
      const headers =
        typeof entry?.headers === "string" ? entry.headers.trim() : undefined;
  ...
  #buildMcpClientConfig(
    ...
    return {
      ...
      requestTimeoutMs: Math.round(
        DEFAULT_MCP_CONFIG.requestTimeoutSeconds * 1000,
      ),
      servers,
    };
  }
  ...
  private async doInitialize(
    ...
    const capabilities =
      mcpConfig.capabilities ?? DEFAULT_MCP_CONFIG.capabilities;
    ...
    if (normalized.properties && typeof normalized.properties === "object") {
      ...
      for (const [key, prop] of Object.entries(props)) {
        ...
        if (normalizedProp.properties)
          Object.assign(
            normalizedProp,
            this.normalizeSchemaProperties(
              normalizedProp as Record<string, unknown>,
            ),
          );
  ...
  private parseHeaders(
    ...
    try {
      ...
      console.warn(
        "[MCPStore] Failed to parse custom headers JSON:",
        headersJson,
      );
    }
   ...
   */
  private promoteHealthCheckToConnection(
    serverId: string,
  ...
  async subscribeToResource(uri: string): Promise<boolean> {
    ...
    try {
      ...
      console.error(
        `[MCPStore] Failed to subscribe to resource ${uri}:`,
        error,
      );
  ...
  async unsubscribeFromResource(uri: string): Promise<boolean> {
    ...
    try {
      ...
      console.error(
        `[MCPStore] Failed to unsubscribe from resource ${uri}:`,
        error,
      );
...
// Resources exports
export const mcpHasResourcesCapability = () =>
