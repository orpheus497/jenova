<script lang="ts">
	...
	async function loadTrash() {
		isLoading = true;
		try {
			trashItems = await FileSystemService.getTrash() || [];
		} catch (e) {
	...
	<div class="flex flex-col gap-4">
		...
		<div class="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-4 gap-4">
			<div>
				...
				<p class="text-on-surface-variant mt-2 font-mono text-sm">Manage deleted workspaces, projects, folders, and assets.</p>
			</div>
			<div class="flex gap-4">
				<button 
					onclick={() => showEmptyDialog = true}
					disabled={isLoading || isActioning || trashItems.length === 0}
