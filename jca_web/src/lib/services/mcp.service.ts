...
export class MCPService {
   ...
   * When `useProxy` is enabled, routes HTTP requests through jenova-server's CORS proxy.
   *
   * **Fallback Order:**
  ...
  static createTransport(config: MCPServerConfig): {
    ...
    if (config.transport === MCPTransportType.WEBSOCKET) {
      ...
      if (import.meta.env.DEV) {
        console.log(
          `[MCPService] Creating WebSocket transport for ${url.href}`,
        );
      }
    ...
    if (useProxy && import.meta.env.DEV) {
      console.log(
        `[MCPService] Using CORS proxy for ${config.url} -> ${url.href}`,
      );
    }
    ...
    try {
      if (import.meta.env.DEV) {
        console.log(
          `[MCPService] Creating StreamableHTTP transport for ${url.href}`,
        );
      }
   ...
   */
  private static extractServerInfo(
    ...
    return {
      ...
      icons: impl.icons?.map(
        (icon: { src: string; mimeType?: string; sizes?: string }) => ({
          src: icon.src,
  ...
  static async connect(
    ...
    const { transport, type: transportType } =
      this.createTransport(serverConfig);
  ...
  private static formatSingleContent(content: ToolResultContentItem): string {
    ...
    if (content.type === MCPContentType.IMAGE && content.data) {
      return createBase64DataUrl(
        content.mimeType ?? DEFAULT_IMAGE_MIME_TYPE,
        content.data,
      );
    }
  ...
  static async complete(
    connection: MCPConnection,
    ref:
      | { type: MCPRefType.PROMPT; name: string }
      | { type: MCPRefType.RESOURCE; uri: string },
   ...
   */
  static async subscribeResource(
    connection: MCPConnection,
    uri: string,
  ): Promise<void> {
    try {
   ...
   */
  static async unsubscribeResource(
    connection: MCPConnection,
    uri: string,
  ): Promise<void> {
    try {
