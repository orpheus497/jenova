...
export class PropsService {
	 ...
	 */
	static async fetch(autoload = false): Promise<ApiJenovaCppServerProps> {
		const params: Record<string, string> = {};
		if (!autoload) {
			params.autoload = 'false';
		}

		return apiFetchWithParams<ApiJenovaCppServerProps>('./props', params, { authOnly: true });
	}
	 ...
	 */
	static async fetchForModel(modelId: string, autoload = false): Promise<ApiJenovaCppServerProps> {
