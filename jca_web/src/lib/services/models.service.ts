  ...
  static parseModelId(modelId: string): ParsedModelId {
    ...
    if (!result.quantization && segments.length > 1) {
      ...
      if (MODEL_QUANTIZATION_SEGMENT_RE.test(last)) {
        if (
          secondLast &&
          MODEL_CUSTOM_QUANTIZATION_PREFIX_RE.test(secondLast)
        ) {
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      if (paramsIdx === MODEL_ID_NOT_FOUND && MODEL_PARAMS_RE.test(seg)) {
        ...
        paramsIdx !== MODEL_ID_NOT_FOUND &&
        MODEL_ACTIVATED_PARAMS_RE.test(seg)
      ) {
    ...
    const pivotIdx =
      paramsIdx !== MODEL_ID_NOT_FOUND ? paramsIdx : segments.length;

    result.modelName =
      segments.slice(0, pivotIdx).join(MODEL_ID_SEGMENT_SEPARATOR) || null;
