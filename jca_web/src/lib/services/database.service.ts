...
export class DatabaseService {
	...
	static async createMessageBranch(
		message: Omit<DatabaseMessage, 'id' | 'parent' | 'children'> & { children?: string[] },
	...
	static async deleteWorkspace(id: string): Promise<void> {
		await db.transaction('rw', [db.workspaces, db.projects, db.folders, db.notes, db.fileAssets], async () => {
	...
	static async deleteProject(id: string): Promise<void> {
		await db.transaction('rw', [db.projects, db.folders, db.notes, db.fileAssets], async () => {
	...
	static async deleteFolder(id: string): Promise<void> {
		await db.transaction('rw', [db.folders, db.notes, db.fileAssets], async () => {
	...
	static async createNote(folderId: string | null, title: string, content: string): Promise<DatabaseNote> {
		const note: DatabaseNote = { id: uuid(), folderId, title, content, updatedAt: Date.now() };
	...
	static async createFileAsset(folderId: string | null, name: string, size: number, type: string, content?: string): Promise<DatabaseFileAsset> {
		const fileAsset: DatabaseFileAsset = { id: uuid(), folderId, name, size, type, uploadDate: Date.now(), content };
