<script lang="ts">
  ...
  let isNewChatMode = $derived(
    page.url.searchParams.get("new_chat") === "true",
  );
  let showSidebarByDefault = $derived(
    activeMessages().length > 0 || isLoading(),
  );
  let alwaysShowSidebarOnDesktop = $derived(
    config().alwaysShowSidebarOnDesktop,
  );
  ...
  let chatSettingsDialogInitialSection = $state<
    SettingsSectionTitle | undefined
  >(undefined);
  ...
  $effect(() => {
    ...
    if (enabledServers.length > 0) {
      untrack(() => {
        ...
        mcpStore
          .runHealthChecksForServers(enabledServers, false)
          .catch((error) => {
  ...
  // Background stability / WakeLock
  let wakeLockRef = $state<unknown>(null);
  ...
  $effect(() => {
    ...
    const requestWakeLock = async () => {
      try {
        if ("wakeLock" in navigator) {
          wakeLockRef = await (navigator as unknown as { wakeLock: { request: (type: string) => Promise<unknown> } }).wakeLock.request("screen");
        }
      } catch (err) {
    ...
    return () => {
      if (wakeLockRef) (wakeLockRef as { release: () => void }).release();
