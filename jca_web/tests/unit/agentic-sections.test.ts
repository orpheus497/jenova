...
describe("deriveAgenticSections", () => {
  ...
  it("multi-turn: three turns with tool calls", () => {
    ...
    const sections = deriveAgenticSections(assistant1, [
      tool1,
      assistant2,
      tool2,
      assistant3,
  ...
  it("multi-turn: streaming tool calls on last turn", () => {
    ...
    expect(sections.some((s) => s.type === AgenticSectionType.TOOL_CALL)).toBe(
      true,
    );
    expect(
      sections.some((s) => s.type === AgenticSectionType.TOOL_CALL_STREAMING),
    ).toBe(true);
  });
});
