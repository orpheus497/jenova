...
function stripLegacyContextMarkers(content: string): string {
  return content
    ...
    .replace(
      new RegExp(LEGACY_AGENTIC_REGEX.AGENTIC_TOOL_CALL_BLOCK.source, "g"),
      "",
    )
...
describe("legacy agentic marker stripping (for migration)", () => {
  ...
  it("strips reasoning block independently", () => {
    const input =
      "<<<reasoning_content_start>>>think hard<<<reasoning_content_end>>>Answer.";
  ...
  it("detects legacy markers", () => {
    expect(LEGACY_AGENTIC_REGEX.HAS_LEGACY_MARKERS.test("normal text")).toBe(
      false,
    );
    expect(
      LEGACY_AGENTIC_REGEX.HAS_LEGACY_MARKERS.test(
        "text<<<AGENTIC_TOOL_CALL_START>>>more",
      ),
    ).toBe(true);
    expect(
      LEGACY_AGENTIC_REGEX.HAS_LEGACY_MARKERS.test(
