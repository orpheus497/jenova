...
describe("normalizeModelName", () => {
  it("preserves Hugging Face org/model format (single slash)", () => {
    ...
    expect(normalizeModelName("meta-jenova/Jenova-3.1-8B")).toBe(
      "meta-jenova/Jenova-3.1-8B",
    );
    expect(normalizeModelName("models/model-name-1")).toBe(
      "models/model-name-1",
    );
  });
  ...
  it("extracts filename from multi-segment paths", () => {
    ...
    expect(normalizeModelName("path/to/model/model-name-2")).toBe(
      "model-name-2",
    );
  ...
  it("extracts filename from backslash paths", () => {
    ...
    expect(normalizeModelName("path\\to\\model\\model-name-2")).toBe(
      "model-name-2",
    );
  });
