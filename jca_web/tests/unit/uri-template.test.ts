...
describe("extractTemplateVariables", () => {
  ...
  it("extracts variables with operators", () => {
    ...
    expect(vars).toEqual([
      { name: "path", operator: URI_TEMPLATE_OPERATORS.RESERVED },
    ]);
  });
  ...
  it("handles fragment expansion", () => {
    ...
    expect(vars).toEqual([
      { name: "section", operator: URI_TEMPLATE_OPERATORS.FRAGMENT },
    ]);
  });
  ...
  it("handles path segment expansion", () => {
    const vars = extractTemplateVariables("http://example.com{/path}");
    expect(vars).toEqual([
      { name: "path", operator: URI_TEMPLATE_OPERATORS.PATH_SEGMENT },
    ]);
  });
...
describe("isTemplateComplete", () => {
  it("returns true when all variables are filled", () => {
    expect(isTemplateComplete("file:///{path}", { path: "test.txt" })).toBe(
      true,
    );
  });
  ...
  it("returns false when a variable is missing", () => {
    expect(
      isTemplateComplete("db://{schema}/{table}", { schema: "public" }),
    ).toBe(false);
  });
...
describe("normalizeResourceUri", () => {
  ...
  it("handles file:// URIs", () => {
    expect(normalizeResourceUri("file:///home/user/doc.txt")).toBe(
      "file://home/user/doc.txt",
    );
  });

  it("handles http URIs unchanged", () => {
    expect(normalizeResourceUri("http://example.com/path")).toBe(
