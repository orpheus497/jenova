...
\\] |`;
  ...
  test("escapes isolated $ before digits ($5 → \\$5), but not valid math", () => {
    ...
    expect(output).toBe(
      "This costs \\$5 and this is math $x^2$. \\$100 is money.",
    );
...
\end{cases}$$`;
    ...
    expect(output)
      .toBe(`- Algebraic topology, Homotopy Groups of $\\mathbb{S}^3$:
$$\n\\pi_n(\\mathbb{S}^3) = \\begin{cases}
\\mathbb{Z} & n = 3 \\\\
0 & n > 3, n \\neq 4 \\\\
\\mathbb{Z}_2 & n = 4 \\\\
\\end{cases}\n$$`);
  ...
  test("preserves code blocks", () => {
    const input =
      "Inline code: `sum $total` and block:\n```\ndollar $amount\n```\nEnd.";
    const output = preprocessLaTeX(input);
