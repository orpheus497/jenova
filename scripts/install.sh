...
# Shared FreeBSD detection — populates JENOVA_OS, JENOVA_PKG_MGR,
# JENOVA_VULKAN_OK, JENOVA_GH_ARCH_*, etc. Also gates this script to a FreeBSD
# kernel, so no OS branching is needed below.
...
_VER="$(printf '%s' "$JENOVA_OS_RELEASE" | cut -d. -f1)"
if [ "${_VER:-0}" -ge 15 ] 2>/dev/null; then
    ok "FreeBSD ${JENOVA_OS_RELEASE} — fully supported"
else
    warn "FreeBSD ${JENOVA_OS_RELEASE} — recommended FreeBSD 15+; proceeding with caution"
    WARNINGS=$((WARNINGS + 1))
fi
...
# 3. Dependencies
# ---------------------------------------------------------------------------
# scripts/install-dependencies.sh installs every dependency and fails if any is
# missing. The Makefile runs it before any build target, so by the time this
# script executes they are already present. Re-run it here for the case where
# install.sh is invoked directly rather than through make.
...
if [ "$CLIENT_ONLY" = "1" ]; then
    ...
    if [ -f "$LLAMA_BIN" ]; then
        ...
        warn "  make llama"
        warn "This builds llama.cpp with the Vulkan backend detected on this host."
        warn "CUDA is opt-in: JENOVA_BACKEND=cuda make llama"
        WARNINGS=$((WARNINGS + 1))
    fi
...
_verify_and_copy_bin() {
    ...
    # Refuse to deploy a binary built for another ABI.
    if command -v file >/dev/null 2>&1; then
        _file_info=$(file "$_src")
        echo "$_file_info" | grep -qi "ELF.*FreeBSD" || {
            info "Skipping non-FreeBSD binary: $(basename "$_src")"
            return 0
        }
    fi
...
if [ -f "$_LLAMA_BUILD_BIN" ]; then
    ...
    for _lib in "$JENOVA_ROOT/external/ext_bin/bin/"*.so*; do
...
if [ "$_ON_PATH" = "0" ]; then
    ...
    warn "Add this to your shell rc file (~/.shrc, ~/.cshrc, etc.):"
    warn "  export PATH=\"\$HOME/.local/bin:\$PATH\""
    WARNINGS=$((WARNINGS + 1))
fi

# Install Desktop Entries & Icons (these don't depend on PATH)
...
if [ "$CLIENT_ONLY" = "1" ]; then
    ...
    echo "  the firewall allows port 8080 from this host."
    echo ""
    echo "  Only 8080 (the Intelligence Proxy) is client-facing. The inference"
    echo "  and embedding servers bind loopback and are reached through it —"
    echo "  do not open 8081 or 8082."
else
