...
`external/llama.cpp` is a Git submodule — run `git submodule update --init` after cloning. `external/ext_bin` contains compiled binaries generated by the build process and is not committed to the repository.
