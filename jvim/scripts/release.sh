...
#     - Add date and version to runtime/jvim.appdata.xml
#     - Tag the commit.
...
__RELEASE_MSG="JVIM v${__VERSION}
...
_do_release_commit() {
  ...
  if grep '(NVIM_API_PRERELEASE true)' CMakeLists.txt > /dev/null; then
    ...
    build/bin/jvim --api-info > "test/functional/fixtures/api_level_$__API_LEVEL.mpack"
  ...
  $__sed -i.bk 's,(<releases>),\1\
    <release date="'"${__DATE}"'" version="'"${__VERSION}"'"/>,' runtime/jvim.appdata.xml
  git add runtime/jvim.appdata.xml
  ...
  git tag --sign -a v"${__VERSION}" -m "JVIM v${__VERSION}"
}
...
_do_bump_commit() {
  ...
  rm -f runtime/jvim.appdata.xml.bk
  jvim +'/NVIM_VERSION' +1new +'exe "norm! iUpdate version numbers!!!"' \
    -O CMakeLists.txt
