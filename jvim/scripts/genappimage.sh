...
########################################################################
# Compile jvim and install it into AppDir
########################################################################

# Build and install jvim into the AppImage
...
VERSION=$("$ROOT_DIR"/build/bin/jvim --version | head -n 1 | grep -o 'v.*')
export VERSION
...
cp "$ROOT_DIR/runtime/jvim.appdata.xml" "$APP_DIR/usr/share/metainfo/"
...
unset ARGV0
exec "$(dirname "$(readlink  -f "${0}")")/usr/bin/jvim" ${@+"$@"}
EOF
chmod 755 AppRun
...
export OUTPUT=jvim-linux-"$ARCH_OUTPUT".appimage

# If it's a release generate the zsync file
if [ -n "$TAG" ]; then
  export UPDATE_INFORMATION="gh-releases-zsync|orpheus497|jvim|$TAG|jvim-linux-$ARCH_OUTPUT.appimage.zsync"
fi

# Generate AppImage.
#   - Expects: $ARCH, $APP, $VERSION env vars
#   - Expects: ./$APP.AppDir/ directory
#   - Produces: ./jvim-linux-$ARCH_OUTPUT.appimage
./linuxdeploy-"$ARCH".AppImage --appdir $APP.AppDir -d "$ROOT_DIR"/runtime/jvim.desktop -i \
"$ROOT_DIR/runtime/jvim.png" --output appimage

# Moving the final executable to a different folder so it isn't in the
# way for a subsequent build.

mv "$ROOT_DIR"/build/jvim-linux-"$ARCH_OUTPUT".appimage* "$ROOT_DIR"/build/bin
