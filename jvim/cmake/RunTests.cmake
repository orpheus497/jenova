...
if(NOT DEFINED ENV{JVIM_LOG_FILE})
  set(ENV{JVIM_LOG_FILE} ${BUILD_DIR}/jvim.log)
endif()
set(ENV{JVIM_LOG_FILE} "$ENV{JVIM_LOG_FILE}${TEST_SUFFIX}")

if(JVIM_PRG)
  set(ENV{JVIM_PRG} "${JVIM_PRG}")
endif()
...
execute_process(
  ...
  COMMAND ${JVIM_PRG} -ll ${ROOT_DIR}/test/lua_runner.lua ${DEPS_INSTALL_DIR}/share/lua/5.1/ busted -v -o test.busted.outputHandlers.jvim
...
macro(PRINT_NVIM_LOG)
  file(READ $ENV{JVIM_LOG_FILE} out)
  if(${TEST_SUMMARY_FILE} STREQUAL "-")
    message(STATUS "$JVIM_LOG_FILE: $ENV{JVIM_LOG_FILE}\n${out}")
  else()
    file(APPEND ${TEST_SUMMARY_FILE} "$JVIM_LOG_FILE: $ENV{JVIM_LOG_FILE}\n${out}")
  endif()
endmacro()
...
if(res)
  ...
  if(CI_BUILD)
    if(EXISTS $ENV{JVIM_LOG_FILE} AND NOT EXISTS $ENV{JVIM_LOG_FILE}.displayed)
      PRINT_NVIM_LOG()
    endif()
  endif()
...
if(CI_BUILD)
  file(SIZE $ENV{JVIM_LOG_FILE} FILE_SIZE)
  if(NOT ${FILE_SIZE} MATCHES "^0$")
    PRINT_NVIM_LOG()
    message(FATAL_ERROR "$JVIM_LOG_FILE is not empty")
  endif()
endif()
