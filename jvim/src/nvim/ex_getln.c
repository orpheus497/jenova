...
static int command_line_execute(VimState *state, int key)
{
  ...
  if (s->c == cedit_key || s->c == K_CMDWIN) {
    // Check "ex_normal_busy" to prevent opening the command-line window when
    // executing a :normal command via 'cedit'. "K_CMDWIN" is specifically
    // used for "q:", "q/" and "q?" and should always open the window.
    // open_cmdwin() will still check "text_or_buf_locked()", which includes
    // a check for "textlock" (set during ":normal").
