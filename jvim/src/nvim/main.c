...
#endif
{
  ...
  if (!appname_is_valid()) {
    fprintf(stderr, "$JVIM_APPNAME must be a name or relative path.\n");
    exit(1);
  }
...
///
/// 1. Source system vimrc file from $XDG_CONFIG_DIRS/$JVIM_APPNAME/sysinit.vim
...
/// 2. Sourcing user config file ($XDG_CONFIG_HOME/$JVIM_APPNAME/init.lua or init.vim).
/// 3. Sourcing other config files ($XDG_CONFIG_DIRS[1]/$JVIM_APPNAME/init.lua or init.vim, …).
  ...
  FUNC_ATTR_WARN_UNUSED_RESULT
{
  ...
  if (L != NULL) {
    ...
    lua_pushstring(L, "jvim_ide");
    if (nlua_pcall(L, 1, 0)) {
      fprintf(stderr, "Failed to load jvim_ide: %s\n", lua_tostring(L, -1));
    }
  }
  // -------------------------------------------
...
static void usage(void)
{
  ...
  printf(_("  --server <address>    Connect to this jvim server\n"));
