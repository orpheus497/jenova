...
///
/// Before the new entry entries from the current jvim history will be
...
/// @param[in]      do_iter         Determines whether jvim own history should
///                                 be used. Must be true only if inserting
///                                 entry from current jvim history.
  ...
  FUNC_ATTR_NONNULL_ALL
{
  ...
  if (val) {
    ...
    if (entry.timestamp > existing_entry->data.timestamp) {
      ...
      // Prefer entry from the current jvim instance.
...
/// Merge in all remaining jvim own history entries
///
/// @param[in,out]  hms_p  Merger structure into which history should be
///                        inserted.
static inline void hms_insert_whole_jvim_history(HistoryMergerState *const hms_p)
  FUNC_ATTR_NONNULL_ALL
{
  while (hms_p->last_hist_entry.type != kSDItemMissing) {
    hms_insert(hms_p, hms_p->last_hist_entry, false);
...
/// Convert merger structure to jvim internal structure for history
///
...
shada_read_main_cycle_end:
  ...
  //          from the history jvim array will no longer be valid. To reduce
  ...
  if (srni_flags & kSDReadHistory) {
    for (int i = 0; i < HIST_COUNT; i++) {
      hms_insert_whole_jvim_history(&hms[i]);
      clr_history(i);
      int *new_hisidx;
      int *new_hisnum;
  ...
  if (dump_history) {
    for (int i = 0; i < HIST_COUNT; i++) {
      if (dump_one_history[i]) {
        hms_insert_whole_jvim_history(&wms->hms[i]);
