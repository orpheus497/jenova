...
/// Initializes the log file path and sets $JVIM_LOG_FILE if empty.
///
/// Tries $JVIM_LOG_FILE, or falls back to $XDG_STATE_HOME/jvim/jvim.log. Failed
/// initialization indicates either a bug in expand_env() or both $JVIM_LOG_FILE
...
static void log_path_init(void)
{
  ...
  if (!user_set
    ...
    if (user_set) {  // User-provided $JVIM_LOG_FILE.
      // Used by _core/log.lua:check_log_file to validate logfile on startup.
      os_setenv("__JVIM_LOG_FILE_WANT", log_file_path, true);
    }
    ...
    XFREE_CLEAR(loghome);
    // Invalid $JVIM_LOG_FILE or failed to expand; fall back to default.
    char *defaultpath = stdpaths_user_state_subpath("jvim.log", 0, true);
    size_t len = xstrlcpy(log_file_path, defaultpath, size);
    xfree(defaultpath);
    // Fall back to $CWD/jvim.log
    if (len >= size || !log_try_create(log_file_path)) {
      if (!user_set) {  // Default fallback path.
        // Used by _core/log.lua:check_log_file to validate logfile on startup.
        os_setenv("__JVIM_LOG_FILE_WANT", log_file_path, true);
      }
      len = xstrlcpy(log_file_path, "jvim.log", size);
    }
    // Fall back to stderr
    if (len >= size || !log_try_create(log_file_path)) {
      log_file_path[0] = NUL;
...
/// Logs a message to $JVIM_LOG_FILE.
///
...
{
  // Name of the jvim instance that produced the log.
  ...
  // Get a name for this jvim instance.
