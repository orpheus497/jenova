...
/// Gets the value of $JVIM_APPNAME, or "jvim" if not set.
///
/// @param namelike Write "name-like" value (no path separators) in `NameBuff`.
///
/// @return $JVIM_APPNAME value
const char *get_appname(bool namelike)
{
  const char *env_val = os_getenv_noalloc("JVIM_APPNAME");

  if (!env_val) {
    xstrlcpy(NameBuff, "jvim", sizeof(NameBuff));
  }
...
///
/// Windows: Uses "…/$JVIM_APPNAME-data" for kXDGDataHome to avoid storing
/// configuration and data files in the same path. #4403
///
/// @param[in]  idx  XDG directory to use.
///
/// @return [allocated] "{xdg_directory}/$JVIM_APPNAME"
...
///
/// @return [allocated] `$XDG_CACHE_HOME/$JVIM_APPNAME/{fname}`
...
/// @param[in]  fname  New component of the path.
///
/// @return [allocated] `$XDG_CONFIG_HOME/$JVIM_APPNAME/{fname}`
...
/// @param[in]  fname  New component of the path.
///
/// @return [allocated] `$XDG_DATA_HOME/$JVIM_APPNAME/{fname}`
...
///
/// @return [allocated] `$XDG_STATE_HOME/$JVIM_APPNAME/{fname}`.
