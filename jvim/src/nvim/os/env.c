...
///
/// Examples (path = /usr/local/share/jvim/runtime/doc/help.txt):
///
///   pend    = help.txt
///   dirname = doc
///   -> doc/help.txt
...
char *vim_getenv(const char *name)
{
  ...
  if (vim_path == NULL) {
    ...
    // Find runtime path relative to the jvim binary: ../share/jvim/runtime
    if (vim_path == NULL) {
      vim_get_prefix_from_exepath(exe_name);
      if (append_path(exe_name, "share/jvim/runtime/", MAXPATHL) == OK) {
        vim_path = exe_name;
      }
    }
