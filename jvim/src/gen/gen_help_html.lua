...
--   1. :helptags ALL
--   2. jvim -V1 -es +"lua require('src.gen.gen_help_html').run_validate()" +q
--   3. jvim -V1 -es +"lua require('src.gen.gen_help_html').test_gen()" +q
--
-- USAGE (GENERATE HTML):
--   1. `:helptags ALL` first; this script depends on vim.fn.taglist().
--   2. jvim -V1 -es --clean +"lua require('src.gen.gen_help_html').gen('./runtime/doc', 'target/dir/')" +q
--      - Read the docstring at gen().
--   3. cd target/dir/ && jekyll serve --host 0.0.0.0
--   4. Visit http://localhost:4000/…/help.txt.html
--
-- USAGE (VALIDATE):
--   1. jvim -V1 -es +"lua require('src.gen.gen_help_html').validate('./runtime/doc')" +q
--      - validate() is 10x faster than gen(), so it is used in CI.
--   2. Check for unreachable URLs:
--      jvim -V1 -es +"lua require('src.gen.gen_help_html').validate('./runtime/doc', true)" +q
--
-- SELF-TEST MODE:
--   1. jvim -V1 -es +"lua require('src.gen.gen_help_html')._test()" +q
--
-- NOTES:
...
local function ensure_runtimepath()
  if not vim.o.runtimepath:find('build/lib/jvim/') then
    vim.cmd [[set runtimepath^=./build/lib/jvim/]]
  end
end
...
function M.gen(help_dir, to_dir, include, commit, parser_path)
  ...
  if parser_path then
    ...
    --    /usr/local/lib/jvim/parser/vimdoc.so
    --    ./build/lib/jvim/parser/vimdoc.so
