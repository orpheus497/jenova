...
-- Does the same as `jvim -c "helptags ++t doc" -c quit`
-- without spawning that command as a separate "jvim" binary, which helps for
-- cross-compiling. This script still must run in a Lua host that provides the
-- `vim` global (for example via `jvim -l ...`).
local out = arg[1]
local dir = arg[2]
