- **IMPORTANT**: Before upgrading to a new version, **always check for breaking changes** in `runtime/doc/news.txt`.


## Quick start

1. Install [build prerequisites](#build-prerequisites) on your system
2. `git clone https://github.com/orpheus497/jvim`
3. `cd jvim`
...
5. `sudo make install`
    ...
    - On Debian/Ubuntu, instead of `sudo make install`, you can try `cd build && cpack -G DEB && sudo dpkg -i jvim-linux-<arch>.deb` (with `<arch>` either `x86_64` or `arm64`) to build DEB-package and install it. This helps ensure clean removal of installed files.

**Notes**:
- From the repository's root directory, running `make` will download and build all the needed dependencies and put the `jvim` executable in `build/bin`.
- Third-party dependencies (libuv, LuaJIT, etc.) are downloaded automatically to `.deps/`.
- After building, you can run the `jvim` executable without installing it by running `VIMRUNTIME=runtime ./build/bin/jvim`.
- Install [Ninja](https://ninja-build.org/) for faster builds. It will automatically be used.
- Install [ccache](https://ccache.dev/) for faster rebuilds. It's used by default. To disable it, use `CCACHE_DISABLE=true make`.
...
- `RelWithDebInfo` ("Release With Debug Info"): Enables many optimizations and adds enough debug info so that when jvim ever crashes, you can still get a backtrace.
...
Afterwards, the `jvim` executable can be found in `build/bin`. To verify the build type after compilation, run:

```bash
./build/bin/jvim --version | grep ^Build
```

To install the executable to a certain location, use:

```bash
make CMAKE_INSTALL_PREFIX=$HOME/local/jvim install
```
...
By default (`USE_BUNDLED=1`), jvim downloads and statically links its needed dependencies. In order to be able to use a debugger on these libraries, you might want to compile them with debug information as well:
...
2. Open the jvim project folder.
    - Visual Studio should detect the cmake files and automatically start building...
3. Choose the `jvim.exe (bin\jvim.exe)` target and hit F5.
...
4. If you see an error like `uv.dll not found`, try the `jvim.exe (Install)` target. Then switch back to `jvim.exe (bin\jvim.exe)`.
...
2. Open the jvim project in CLion.
...
1. Install all dependencies the normal way.
  ...
  - https://github.com/cascent/neovim-cygwin was built on Cygwin 2.9.0. Newer `libuv` should require slightly less patching. Some SSP stuff changed in Cygwin 2.10.0, so that might change things too when building jvim.
...
3. You have two options:
    - Build using `cmake` and `Ninja` generator:
      ...
      cmake --build build
      ```
      If you cannot install jvim with `ninja install` due to permission restriction, you can install jvim in a directory you have write access to.
      ```cmd
      cmake -S cmake.deps -B .deps -G Ninja -D CMAKE_BUILD_TYPE=RelWithDebInfo
      cmake --build .deps
      cmake -B build -G Ninja -D CMAKE_INSTALL_PREFIX=C:\jvim -D CMAKE_BUILD_TYPE=RelWithDebInfo
      cmake --build build
      ```
    - Or, alternatively, you can use `mingw32-make`:
      ...
      :: (Default is C:\Program Files (x86)\jvim)
      :: mingw32-make CMAKE_BUILD_TYPE=RelWithDebInfo CMAKE_INSTALL_PREFIX=C:\jvim
...
cd build && cpack -G DEB && sudo dpkg -i jvim-linux-<arch>.deb

# Verify the installation
jvim --version && which jvim # should be debug build in /usr/bin/jvim
```
...
Translations are turned off by default. Enable by building jvim with the CMake flag `ENABLE_TRANSLATIONS=ON`.
...
- `check-po-$LANG` generates a detailed report in `./build/src/nvim/po/check-${LANG}.log`. (The report is generated by `jvim`, not by `msgfmt`.)
...
For reference on dependency versions, see upstream [jvim Debian package](https://packages.debian.org/sid/source/neovim) or [jvim Homebrew formula](https://github.com/Homebrew/homebrew-core/blob/master/Formula/n/neovim.rb).
...
By default the libraries and headers are placed in `.deps/usr`. Now you can build jvim:
...
4. If treesitter parsers are not bundled, they need to be available in a `parser/` runtime directory (e.g. `/usr/share/jvim/runtime/parser/`).
...
The resulting binary in `build/bin/jvim` will have all the dependencies statically linked:

```
build/bin/jvim: ELF 64-bit LSB executable, ARM aarch64, version 1 (SYSV), statically linked, BuildID[sha1]=b93fa8e678d508ac0a76a2e3da20b119105f1b2d, with debug_info, not stripped
```
...
- CMake version 3.16+, built with TLS/SSL support
  - Optional: Get the latest CMake from https://cmake.org/download/
    - Provides a shell script which works on most Linux systems. After running it, ensure the resulting `cmake` binary is in your $PATH so the the jvim build will find it.
...
For NixOS development, you can reference the upstream Neovim packages. The Neovim binary resides in the `neovim-unwrapped` Nix package (the `neovim` package being just a wrapper to setup runtime options):

```bash
cd path/to/jvim/src
```

Drop into `nix-shell` to pull in the dependencies (using jvim's package as a reference):
...
A flake for upstream Neovim is hosted at [nix-community/neovim-nightly-overlay](https://github.com/nix-community/neovim-nightly-overlay/).
...
To use a specific version of jvim, you can pass `--override-input neovim-src .` to use your current directory,
...
3. Install jvim build dependencies:
    ```bash
...
### Instructions
 - Build the editor: `zig build`, run it with `./zig-out/bin/jvim`
