...
local function agent_respond(buf, prompt, on_done, history)
  ...
  local raw_stream_buffer = "" -- raw text accumulator for the current stream
  local stream_scan_cursor = 1
  ...
  local function append_text(text)
    ...
    raw_stream_buffer = raw_stream_buffer .. text

    local tool_start = nil
    local s = stream_scan_cursor
    while true do
      local start_idx, end_idx = raw_stream_buffer:find("```json.-```", s)
      if not start_idx then break end
      
      local content = vim.trim(raw_stream_buffer:sub(start_idx + 7, end_idx - 3))
      local ok, data = pcall(vim.json.decode, content)
      if ok and type(data) == "table" then
        local name = data.name or data.tool
        local ok_reg, reg = pcall(require, "jenova.agent.registry")
        if name and ok_reg and reg and reg.get(name) then
          tool_start = start_idx
          break
        end
      end
      s = end_idx + 1
    ...
    if not tool_start then
      local ok_reg, reg = pcall(require, "jenova.agent.registry")
      if ok_reg and reg then
        local search_area = raw_stream_buffer:sub(stream_scan_cursor)
        for tag_name in search_area:gmatch("<(%w+)[%s>]") do
          if reg.get(tag_name) then
            if raw_stream_buffer:find("<" .. tag_name .. "[^>]*>.-</" .. tag_name .. ">", stream_scan_cursor) then
              tool_start = raw_stream_buffer:find("<" .. tag_name, stream_scan_cursor)
              break
            end
          end
        end
    ...
    stream_scan_cursor = math.max(1, #raw_stream_buffer - 100)

    local text_to_show = raw_stream_buffer
    if tool_start then
      text_to_show = vim.trim(raw_stream_buffer:sub(1, tool_start - 1))
    else
      text_to_show = vim.trim(raw_stream_buffer)
    end

    if text_to_show == "" and tool_start then
      -- If it's purely a tool call with no preceding text, just return
      -- after ensuring transient line is clear (handled below)
      return
    end
    if text_to_show == "" then return end
    ...
    local old_count = stream_lines and #stream_lines or 1
    local pieces = vim.split(text_to_show, "\n", { plain = true })
    if #pieces == 0 then pieces = { "" } end
    stream_lines = pieces

    pcall(vim.api.nvim_buf_set_lines, buf,
      stream_start - 1,
      stream_start - 1 + old_count,
      false, stream_lines)
    scroll_to_bottom(buf)
  end
