...
function M.resolve(path, cwd)
  ...
  local relative_to_cwd = (cwd or vim.fn.getcwd()) .. "/" .. path
  
  -- If it exists relative to cwd, or we are explicitly in cwd, use it
  if vim.fn.filereadable(relative_to_cwd) == 1 or vim.fn.isdirectory(relative_to_cwd) == 1 then
    return relative_to_cwd
  end

  -- Fallback: models often drop ~/ from paths like ~/Documents/...
  -- Check if it exists relative to HOME instead
  local home = vim.fn.expand("~")
  local relative_to_home = home .. "/" .. path
  if vim.fn.filereadable(relative_to_home) == 1 or vim.fn.isdirectory(relative_to_home) == 1 then
