...
function M.build_editor_context(chat_buf)
  ...
  if ws_buf and path ~= "" then
    ...
    table.insert(lines, string.format("active_buffer (%s, %d lines):\n```\n%s\n```",
      abs_path, #all_lines, table.concat(numbered, "\n")))
  end
...
function M.build_file_seed_prompt()
  ...
  return string.format(
    ...
    abs_path, ft, table.concat(numbered, "\n"))
end
...
function M.build_system_prompt(chat_buf, user_message)
  ...
  if ws_buf then
    local p = vim.api.nvim_buf_get_name(ws_buf)
    if p and p ~= "" then
      active_path = vim.fn.fnamemodify(p, ":p")
    end
  end

  local ctx_path = nil
  if chat_buf then
    local metadata = safe(parse_metadata_from_buffer, chat_buf) or {}
    if metadata.path then
      local meta_p = vim.fn.fnamemodify(metadata.path, ":p")
      if meta_p ~= active_path then
        ctx_path = meta_p
      end
    end
  end
  ...
  local injected = {}
  if active_path then table.insert(injected, active_path) end
  if ctx_path then table.insert(injected, ctx_path) end

  local exception = " Read first."
  if #injected > 0 then
    exception = " Read first unless editing: " .. table.concat(injected, ", ") .. "."
  end
  ...
  local base_lines = {
    ...
    "5. MANDATORY TOOL USE: If you output code in a markdown block without calling a tool, you have FAILED the task, UNLESS the user explicitly asks for an explanation or says 'without writing code' or 'just show me'.",
  }
  ...
  if vim.g.jenova_tools_enabled ~= false then
    local tools = {
      ...
      "- Edit(file_path, start_line, end_line, new_string): Replace line range." .. exception,
      "- MultiEdit(file_path, edits[{start_line, end_line, new_string}]): Batch edits." .. exception,
      ...
      "",
      "## Rules",
      "0. JUST ACT. Do not discuss what you are going to do. Issue the tool calls required to complete the task. (Exception: if the user asks a question or asks for explanation, answer directly).",
      ...
      "- You MUST Read a file before any Edit/MultiEdit (UNLESS the file is already provided in the system prompt). Use exact line numbers from the Read output or provided context.",
