...
local function apply_keys(keys, loader)
...
local function run_spec(spec)
  ...
  apply_keys(spec.keys, load_now)
  apply_cmd(spec.cmd, load_now)
  apply_event(spec.event, load_now)
  apply_ft(spec.ft, load_now)

  -- No deferred trigger (event/cmd/ft)? Load eagerly.
  if not (spec.event or spec.cmd or spec.ft) then
    load_now()
  end
end
