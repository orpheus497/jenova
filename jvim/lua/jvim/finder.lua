...
local function rank(items, query, formatter, limit)
  ...
  local scored = {}
  local max_candidates = math.max(limit * 5, 1000)
  for _, it in ipairs(items) do
    local disp = type(it) == "string" and it or formatter(it)
    local s = score(query, disp)
    if s then
      scored[#scored + 1] = { item = it, display = formatter(it), score = s }
      if #scored >= max_candidates then break end
    end
  end
...
local function on_prompt_change()
  ...
  if text:sub(1, #prefix) == prefix then
    picker.query = text:sub(#prefix + 1)
  else
    picker.query = text
  end
  picker.cursor = 1
...
function M.grep(opts)
  ...
  local function live(query)
    ...
    for i, l in ipairs(lines) do
      local file, lnum, col, text = l:match("^([a-zA-Z]:[^\n:]+|[^:]+):(%d+):(%d+):(.*)$")
      if file then
        local disp = string.format("%s:%s:%s %s", vim.fn.fnamemodify(file, ":."), lnum, col, text)
        out[#out + 1] = { item = l, display = disp }
      else
        out[#out + 1] = { item = l, display = l }
      end
  ...
  M.pick({
    ...
    on_pick = function(line)
      ...
      local file, lnum, col = line:match("^([a-zA-Z]:[^\n:]+|[^:]+):(%d+):(%d+):")
