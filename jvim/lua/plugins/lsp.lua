  ...
  -- ##Section purpose: nvim-lspconfig — LSP server setup with FreeBSD support
  {
    ...
    config = function()
      ...
      local function get_cmd(server)
        ...
        elseif server == "nimls" then
          if vim.fn.executable("nimlsp") == 1 then return { "nimlsp" } end
        elseif server == "nim_langserver" then
          if vim.fn.executable("nimlangserver") == 1 then return { "nimlangserver" } end
        end
        return nil
      end

      -- ##Step purpose: Full server list — dynamically select nimls vs nim_langserver
      local nim_server = vim.fn.executable("nimlangserver") == 1 and "nim_langserver" or "nimls"
      local servers = { "clangd", "rust_analyzer", "gopls", "pyright", "zls", "bashls", "lua_ls", nim_server }
      ...
      for _, server in ipairs(servers) do
        ...
        if vim.lsp.config then
          ...
          if ok_lsp and lspconfig[server] then
      ...
      vim.api.nvim_create_autocmd("LspAttach", {
        ...
        callback = function(ev)
          ...
          -- ##Step purpose: Enable native inlay hints if supported by Neovim
