-- Jenova Vim (jvim) — jvim native configuration
...
vim.api.nvim_create_user_command("JenovaMonitor", function()
	...
	if ok then
		...
		vim.notify("Failed to load jenova.monitor", vim.log.levels.ERROR)
	end
		...
		vim.defer_fn(function()
			_jenova_tcp_probe(function(connected)
				...
				if connected then
					...
					local has_jvim_env = (
						vim.env.JENOVA_ROOT
						and vim.env.JENOVA_ROOT ~= ""
						and vim.env.JENOVA_ROOT ~= "$JENOVA_ROOT"
					)
					if is_lan_mode and has_connect_host then
						vim.notify(
							("LAN remote %s:%s not responding."):format(
								vim.env.JENOVA_CONNECT_HOST,
					...
					elseif has_jvim_env then
						vim.notify(
							"Jenova CA not running. AI features unavailable.",
							vim.log.levels.WARN,
							{ title = "Jenova" }
						)
						local ok, monitor = pcall(require, "jenova.monitor")
...
if _jenova_timer then
	_jenova_timer:start(
		...
		vim.schedule_wrap(function()
			...
			if _cached_monitor and _cached_monitor._timer then
				return
			end
			_jenova_tcp_probe(function(connected)
				vim.g.jenova_connected = connected
	...
	vim.api.nvim_create_autocmd("VimLeavePre", {
		...
		callback = function()
			...
			local ok, monitor = pcall(require, "jenova.monitor")
			if ok and monitor.stop_polling then
