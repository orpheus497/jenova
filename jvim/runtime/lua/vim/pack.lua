...
function M.get(names, opts)
  ...
  local names_set --- @type table<string,boolean>?
  if names then
    names_set = {}
  ...
  for i = 1, n_active_plugins do
    if active[i] and (not names_set or names_set[active[i].spec.name]) then
  ...
  for name, l_data in vim.spairs(plugin_lock.plugins) do
    local path = vim.fs.joinpath(plug_dir, name)
    local is_in_names = not names_set or names_set[name]
