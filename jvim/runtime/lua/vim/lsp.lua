...
function lsp.get_configs(filter)
  ...
  for _, config_name in ipairs(config_names) do
    ...
    if config and (filter.enabled ~= false or not lsp.is_enabled(config_name)) then
      local match_filetype = filter.filetype == nil
      if not match_filetype and config.filetypes ~= nil then
        if not config._filetypes_set then
          config._filetypes_set = {}
          for _, ft in ipairs(config.filetypes) do
            config._filetypes_set[ft] = true
          end
        end
        match_filetype = config._filetypes_set[filter.filetype]
      end
      if match_filetype then
