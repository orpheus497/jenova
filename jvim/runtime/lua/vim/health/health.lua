...
local suggest_faq = 'https://github.com/orpheus497/jvim/blob/main/BUILD.md'
...
local function check_stable_version(nvim_version)
  local result = vim
    .system(
      { 'git', 'ls-remote', '--tags', 'https://github.com/orpheus497/jvim' },
  ...
  if current and latest and vim.version.lt(current, latest) then
    vim.health.warn(('jvim %s is available (current: %s)'):format(latest_version, current_version))
  else
...
local function check_head_hash(commit)
  local result = vim
    .system(
      { 'git', 'ls-remote', 'https://github.com/orpheus497/jvim', 'HEAD', 'refs/tags/nightly' },
      { text = true, timeout = 5000 }
    )
    :wait()
  if result.code ~= 0 or not result.stdout or result.stdout == '' then
...
local function check_sysinfo()
  ...
  vim.health.info(('jvim version: `%s` %s'):format(nvim_version, commit))
  ...
  local body = vim.text.indent(
    ...
    - jvim version (jvim -v): `%s` orpheus497/jvim@%s
    - Vim (not jvim) behaves the same?: ?
  ...
  vim.api.nvim_create_autocmd('FileType', {
    ...
    callback = function(ev)
      ...
      local issue_url = 'https://github.com/orpheus497/jvim/issues/new?type=Bug&body=' .. encoded_body
