...
# Tests: config loading, health check, agent startup/shutdown, cleanup guard
#
# Prerequisites:
...
if [ -f "$PID_FILE" ]; then
    ...
    # PID file should contain space-separated numeric PIDs, no "0" placeholders
    if echo "$PID_CONTENT" | grep -q "^[0-9 ]*$" && ! echo "$PID_CONTENT" | grep -qw "^0$"; then
