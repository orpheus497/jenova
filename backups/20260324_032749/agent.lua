...
1. NEVER create, write, modify, or delete ANY file unless the user EXPLICITLY asked you to. "Investigate", "list", "check", "look at", "review" = READ ONLY.
2. NEVER claim a file exists or doesn't exist without actually reading it or listing the directory first. If you haven't called read_file or list_dir, you don't know — call the tool.
3. NEVER fabricate or guess file contents. If you need to know what's in a file, call read_file.
4. Call tools immediately — do NOT say "I will check" or "let me look". Just call the tool.
5. After receiving a tool result, continue with the next step. Do NOT stop to narrate.
6. When investigating, read ALL relevant files before responding.
7. When asked to create or modify files, do so. But ONLY when asked.
8. If a tool returns "BLOCKED", the user denied that action. Acknowledge it and move on. Do NOT retry the same blocked action.
...
- `search_files`: Search project files by content keywords (BM25). Use to find relevant files before reading them.

## Strategy
1. Before acting on a complex task, briefly state your plan (1-2 lines).
2. Execute ALL needed tool calls — do not stop after just one.
3. If a tool returns an error, try to fix the issue and retry.
4. After completing all steps, give a concise summary of what was done.
5. ALWAYS verify claims by calling tools. Never say "the file contains X" without having read it.]]
...
local function agent_turn(user_msg)
  ...
  while turn < MAX_TURNS do
    ...
    if tool_calls and #tool_calls > 0 then
      turn = turn + 1
      nudge_count = 0
      io.write(C.yellow .. "  [tools: " .. #tool_calls .. " call(s), turn " .. turn .. "/" .. MAX_TURNS .. "]" .. C.reset .. "\n")

      for _, tc in ipairs(tool_calls) do
        local fn = tc["function"] or tc
        local name = fn.name or tc.name
        local arguments = fn.arguments or tc.arguments
        local tool_call_id = tc.id or ("call_" .. tostring(turn) .. "_" .. tostring(_))
