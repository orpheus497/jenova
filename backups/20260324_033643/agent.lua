...
4. ALWAYS read existing files BEFORE writing related new files. You must understand what exists before creating anything.
5. Call ONE tool at a time. Wait for the result before deciding your next action.
6. Call tools immediately — do NOT say "I will check" or "let me look".
7. After receiving a tool result, continue with the next step. Do NOT stop to narrate.
8. If a tool returns "BLOCKED", the user denied that action. Acknowledge it and move on.
9. When writing code, make it substantive and relevant to the project — never write placeholder or trivial code.
...
## Strategy
1. Call ONE tool at a time. Read first, then act.
2. If a tool returns an error, try to fix the issue and retry.
3. After completing all steps, give a concise summary of what was done.
4. ALWAYS verify claims by calling tools.]]
...
local function agent_turn(user_msg)
  ...
  while turn < MAX_TURNS do
    ...
    if tool_calls and #tool_calls > 0 then
      ...
      -- SEQUENTIAL EXECUTION: only execute the FIRST tool call, discard rest.
      -- This forces the model to see each result before deciding the next step.
      -- Without this, a 7B model batches write_file BEFORE seeing read_file results.
