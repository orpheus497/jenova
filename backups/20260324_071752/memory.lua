...
function memory.get_project_tree(root, max_depth)
  ...
  local cmd = string.format(
    "find %s -maxdepth %d -type f -not -path '*/.git/*' -not -path '*/.coder/*' -not -path '*/.crush/*' -not -path '*/node_modules/*' -not -path '*/__pycache__/*' -not -path '*/build/*' -not -path '*/backups/*' -not -path '*/llama.cpp/*' -not -name '*.gguf' -not -name '*.bin' -not -name '*.o' -not -name '*.so' 2>/dev/null | head -200 | sort",
    root, max_depth
  )
