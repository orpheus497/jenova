...
-- * Qwen2.5-Coder-7B NEVER returns structured tool_calls via the API.
--   All tool use arrives as text in msg.content. We extract it via fallback
--   parser (Stage 2) or code-block interceptor (Stage 3).
-- * The 7B context budget is precious. Every token of system prompt, tool
--   result, and nudge message competes with the model's reasoning capacity.
--   Keep all injected text as SHORT as possible.
...
local CURL_TIMEOUT = tonumber(os.getenv("CODER_TIMEOUT")) or 300
...
-------------------------------------------------------------------------------
-- System prompt
-- DESIGN: Keep this as small as possible. Every word costs the 7B model
-- reasoning capacity. Show-by-example > explain-by-rules.
-- The tool definitions (TOOLS table) already describe what each tool does;
-- don't repeat that info here.
-------------------------------------------------------------------------------
local rag_context = ""

local function build_system_prompt()
  local parts = {}
  parts[#parts+1] = "You are coder, an autonomous coding agent. You work in: " .. (CWD or ".")
  parts[#parts+1] = [[
RESPOND ONLY WITH TOOL CALL JSON. No narration. No explanation. No code blocks.

Workflow — follow this order:
1. read_file to see current content
2. shell to compile/check errors: {"name":"shell","arguments":{"command":"cc -fsyntax-only -Wall file.c 2>&1"}}
3. shell to check what headers/libs exist: {"name":"shell","arguments":{"command":"ls /usr/include/ /usr/local/include/ 2>&1 | head -40"}}
4. edit_file to fix ALL issues (not just one). Fix includes, types, function signatures.
5. shell to compile again — verify clean build
6. Repeat until zero errors

Critical rules:
- NEVER use headers or types you have not verified exist on this system.
- ALWAYS compile after editing to check your work.
- Use edit_file for changes. Use write_file ONLY for new files.
- Use relative paths.
- If edit fails, use read_file to see current content, then retry.
- Fix ALL errors in one edit, not one at a time.
- When fully done, say what you changed in 1-2 sentences.

Example flow:
{"name":"read_file","arguments":{"path":"main.c"}}
{"name":"shell","arguments":{"command":"cc -fsyntax-only -Wall main.c 2>&1"}}
{"name":"edit_file","arguments":{"path":"main.c","old":"#include <fake.h>","new":"#include <real.h>"}}
{"name":"shell","arguments":{"command":"cc -fsyntax-only -Wall main.c 2>&1"}}]]
...
local TOOLS = {
  ...
  { type = "function", ["function"] = {
    ...
    parameters = { type = "object",
      ...
      required = {} } } },
  { type = "function", ["function"] = {
    name = "search_files", description = "Search project files by keyword",
    parameters = { type = "object",
      properties = {
...
local function exec_search_files(args)
  ...
  local results = search.query(query, args.top_k or 5)
...
local function parse_tool_calls_from_content(content)
  ...
  -- 2) Bare JSON: {"name":"tool","arguments":{...}}
...
local function http_post(url, body)
  ...
  local tmp_resp = os.tmpname()
  local f = io.open(tmp_body, "w"); f:write(body); f:close()
  local cmd = string.format(
    'curl -s -S --max-time %d -X POST -H "Content-Type: application/json" -d @%s -o %s -w "%%{http_code}" %s 2>&1',
    CURL_TIMEOUT, tmp_body, tmp_resp, url)
  local p = io.popen(cmd)
  local out = p:read("*a"); p:close()
  os.remove(tmp_body)
  local rf = io.open(tmp_resp, "r")
  local resp = rf and rf:read("*a") or ""
  if rf then rf:close() end
  os.remove(tmp_resp)
  local code = tonumber(out:match("(%d%d%d)$") or "0")
  return code, resp
end
...
local function trim_messages(max)
  ...
  for i = math.max(2, #messages - max + 2), #messages do
...
local function chat(user_msg)
  ...
  local body = json.encode({ model = MODEL, messages = send, tools = TOOLS, tool_choice = "auto" })
  dbg("req", body)
  ...
  if code == 0 then
    status(ICON.err, C.red, C.red.."timeout ("..CURL_TIMEOUT.."s)"..C.reset)
    return nil
  end
  if code ~= 200 then
...
local function check_server()
  local p = io.popen("curl -s -o /dev/null -w '%{http_code}' --max-time 5 "..API_URL.."/health 2>/dev/null")
  local code = p:read("*a"); p:close()
  return code == "200"
end
...
local function main()
  ...
  io.write(C.dim.."  "..API_URL.." | turns:"..MAX_TURNS.." actions:"..MAX_ACTIONS.." timeout:"..CURL_TIMEOUT.."s | BM25:"..indexed.." "..emb..C.reset.."\n")
  io.write(C.dim.."  /clear /history /debug /context /reindex /files /search /errors /quit"..C.reset.."\n\n")
